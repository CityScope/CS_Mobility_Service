...
def get_location(longitude, latitude, regions_json, name): 
    ...
    for record in regions_json['features']:
        ...
        if polygon.contains(point):
            return int(record['properties'][name])
    return 'None'
...
RECENT_MOVERS_DF_PATH=city+'/clean/recent_movers.csv'
VACANT_HOUSING_DF_PATH=city+'/clean/vacant_housing.csv'
...
for f in zones_shp['features']:
    ...
    med=shape(f['geometry']).centroid
    bg_to_puma[f['properties']['GEO_ID']]=get_location(med.x, med.y, pumas_shape, 'PUMACE10')
...
# =============================================================================
# Load and process the PUMS data 
# =============================================================================
...
renters_recent_move=renters_recent_move[['PUMA','HINCP',  'norm_rent', 'RNTP', 'less_than_5yrs_old', 'puma_pop_per_sqmeter', 'med_income']]
hh_vacant_for_rent=hh_vacant_for_rent[['PUMA', 'HINCP', 'norm_rent', 'RNTP','less_than_5yrs_old', 'puma_pop_per_sqmeter', 'med_income']]

renters_recent_move.to_csv(RECENT_MOVERS_DF_PATH, index=False)
hh_vacant_for_rent.to_csv(VACANT_HOUSING_DF_PATH, index=False)
#********************************************
...
for i, row in sample_ind.iterrows():
    ...
    new_person['home_PUMA']=bg_to_puma[row['h_block_group']]
