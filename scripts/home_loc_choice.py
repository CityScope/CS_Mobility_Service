...
hh_choice_set=hh[((hh['TEN']==3)|(hh['VACS']==1))]
...
hh_choice_set.loc[hh_choice_set['BDSP']>2, 'BDSP']=3
hh_choice_set.loc[hh_choice_set['BDSP']<1, 'BDSP']=1
rent_mean={}
rent_std={}
...
for df in [renters_recent_move, hh_choice_set]:
...
hh_choice_set=hh_choice_set[['PUMA', 'HINCP', 'norm_rent', 'RNTP','built_since_jan2010', 'puma_pop_per_sqmeter', 'med_income']]
...
for ind_actual, row_actual in renters_recent_move[:sample_size].iterrows():
    ...
    for i in range(NUM_ALTS):
        selected=random.choice(range(len(hh_choice_set)))
        alt_obs={'custom_id':ind,# identify the individual
                 'choice_id':cid, # fake choice identifier- shouldn't matter if no ASC
                 'choice':0,
                 'rent':hh_choice_set.iloc[selected]['RNTP'],
                 'norm_rent':hh_choice_set.iloc[selected]['norm_rent'],
                 'puma':hh_choice_set.iloc[selected]['PUMA'],
                 'built_since_jan2010':int(hh_choice_set.iloc[selected]['built_since_jan2010']),
                 'puma_pop_per_sqmeter': hh_choice_set.iloc[selected]['puma_pop_per_sqmeter'],
                 'med_income': hh_choice_set.iloc[selected]['med_income'],
                 'hh_income':row_actual['HINCP']
                 }
        cid+=1
